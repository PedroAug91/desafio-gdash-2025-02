...
export class DefaultUserCreatorService implements OnModuleInit {
    ...
    async onModuleInit() {
        ...
        if (!email || !password) {
            this.logger.warn('Default user creation skipped: Missing EMAIL or PASSWORD environment variables.');
            return;
        }
        ...
        if (existingUser) {
            this.logger.log(`Default user with email: ${email} already exists. Skipping creation.`);
            return;
        }
